  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={styles.plan}>
            {item.plan.map((step) => (
              <View key={step.id} style={styles.planRow}>
                <Text style={[styles.stepMark, step.state === 'active' && styles.activeMark]}>
        ...
        <Pressable style={styles.tellButton} onPress={() => router.push('/capture')}>
          <Text style={styles.tellButtonText}>●  Tell Carry something</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  link: { color: colours.rust, fontWeight: '700' },
  state: { color: colours.moss, fontSize: 12, fontWeight: '800', letterSpacing: 1.4, marginTop: spacing.lg },
  title: { color: colours.ink, fontSize: 38, lineHeight: 42, fontWeight: '750', letterSpacing: -1.5, marginTop: spacing.xs },
  ...
  decisionText: { color: colours.ink, fontSize: 20, lineHeight: 27, fontWeight: '650' },
  actions: { flexDirection: 'row', gap: spacing.sm, marginTop: spacing.sm },
  primaryButton: { backgroundColor: colours.ink, borderRadius: radius.pill, paddingHorizontal: spacing.md, paddingVertical: 12 },
  primaryButtonText: { color: colours.white, fontWeight: '700' },
  secondaryButton: { borderColor: colours.ink, borderWidth: 1, borderRadius: radius.pill, paddingHorizontal: spacing.md, paddingVertical: 12 },
  secondaryButtonText: { color: colours.ink, fontWeight: '700' },
  block: { marginTop: spacing.xl, borderTopWidth: 1, borderTopColor: colours.line, paddingTop: spacing.lg },
  blockTitle: { color: colours.ink, fontSize: 17, fontWeight: '750', marginBottom: spacing.md },
  nowText: { color: colours.secondaryInk, fontSize: 17, lineHeight: 25 },
  plan: { gap: spacing.md },
  planRow: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm },
  stepMark: { color: colours.muted, width: 20, fontSize: 15 },
  activeMark: { color: colours.rust },
  stepText: { flex: 1, color: colours.ink, fontSize: 15, lineHeight: 22 },
  doneText: { color: colours.muted },
  activity: { gap: spacing.md },
  activityRow: { flexDirection: 'row', gap: spacing.md },
  activityTime: { color: colours.muted, fontSize: 12, width: 42, paddingTop: 2 },
  activityCopy: { flex: 1 },
  activityActor: { color: colours.ink, fontSize: 13, fontWeight: '700' },
  activityText: { color: colours.secondaryInk, fontSize: 14, lineHeight: 20, marginTop: 2 },
  tellButton: { marginTop: spacing.xl, backgroundColor: colours.ink, borderRadius: radius.pill, paddingVertical: 15, alignItems: 'center' },
  tellButtonText: { color: colours.white, fontWeight: '700', fontSize: 15 },
});
