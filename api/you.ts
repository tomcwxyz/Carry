...
export async function GET(request: Request) {
  ...
  return Response.json({
    ...
    connections: [
      ...
      { id: 'notifications', label: 'Hand-back notifications', connected: Number(deviceCount?.count ?? 0) > 0, detail: Number(deviceCount?.count ?? 0) > 0 ? 'Carry can notify this device when something genuinely needs you.' : 'Open Carry on a physical device and allow notifications.' },
