...
export default async function handler(request: Request) {
  ...
  try {
    ...
    const [capture] = await sql`INSERT INTO carry_captures (owner_key, kind, status, raw_text, transcript, media_type) VALUES (${ownerKey}, ${kind}, ${kind === 'voice' ? 'transcribed' : 'received'}, ${kind === 'text' ? sourceText : null}, ${kind === 'voice' ? sourceText : null}, ${mediaType}) RETURNING id`;
    if (!capture?.id) throw new Error('Carry could not persist the capture');

    let understood;
    try { understood = await understandCase(sourceText); }
    catch (error) { console.error('case_understanding_failed', error); understood = fallbackCase(sourceText); }

    const plan = withStepIds(understood.plan);
    const decision = understood.decisionLabel ? { label: understood.decisionLabel } : null;
    const [created] = await sql`INSERT INTO carry_cases (owner_key, title, outcome, summary, state, domain, source_text, next_action, decision, plan) VALUES (${ownerKey}, ${understood.title}, ${understood.outcome}, ${understood.summary}, ${understood.state}, ${understood.domain}, ${sourceText}, ${understood.nextAction}, ${decision ? JSON.stringify(decision) : null}::jsonb, ${JSON.stringify(plan)}::jsonb) RETURNING id`;
    if (!created?.id) throw new Error('Carry could not create the case');

    await sql`INSERT INTO carry_case_events (case_id, type, actor, label, payload) VALUES (${created.id}, 'captured', 'you', ${kind === 'voice' ? 'Told Carry about this by voice' : 'Told Carry about this'}, ${JSON.stringify({ kind })}::jsonb), (${created.id}, 'understood', 'carry', 'Turned the capture into an outcome and working plan', '{}'::jsonb)`;
    await sql`UPDATE carry_captures SET status = 'understood', case_id = ${created.id}, updated_at = now() WHERE id = ${capture.id}`;

    if (understood.state === 'carrying') {
      try { await runCase(created.id, ownerKey, { trigger: 'capture', maxSteps: 2 }); }
      catch (error) { console.error('initial_carry_run_failed', error); }
    }
